...
export type BlockchainTransactionResponse = {
  _embedded: {
    records: BlockchainTransactionRecord[];
  };
};
...
const checkPaymentValid = async (
  ...
  try {
    ...
    const onchainAmount = records
      .filter((record) => record.type === "account_credited")
...
const completePaymentWithReward = async (
  ...
  try {
    if (!payment.transaction) {
      console.error("Payment has no transaction data");
      return;
    }
...
const createPaymentCallbacks = (
  ...
  const onReadyForServerCompletion = async (
    ...
    try {
      ...
      await completePaymentWithReward(currentPayment);
...
export const checkIncompletePayments = async (
  ...
  try {
    ...
    if (!payment.transaction) {
      console.log("Incomplete payment has no transaction yet, skipping");
      return;
    }
